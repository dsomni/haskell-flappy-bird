...
drawNumber x = lettering $ toText x

toText :: (Show a) => a -> T.Text
toText int = T.pack $ show int
...
drawLeaderBoard :: [(T.Text, Int)] -> Picture
drawLeaderBoard leaders = translated 0 4 (scaled 1.5 1.5 (lettering "LeaderBoard")) <> translated 0 (fromIntegral (length leaders) / 2) (pictures (map drawLeader (zip [1 ..] leaders))) <> colored purple (solidRectangle 10 12)
  where
    drawLeader :: (Int, (T.Text, Int)) -> Picture
    drawLeader (position, (name, score)) = translated 0 (fromIntegral (-position)) (lettering (toText position <> ".  " <> name <> " " <> toText score))
